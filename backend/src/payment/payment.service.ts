...
export class PaymentService {
  ...
  async placeOrder(data: OrderDto) {
    try {
      ...
      // Get event charge
      const chargeSetup = await this.prisma.chargeSetup.findFirst({
        where: { id: data.eventId },
      });
      ...
      const transactionList = await this.buildTransactions(
        data,
        event,
        batchId,
      );
  ...
  async getTransactionByReg(id: string) {
    try {
      const transactions = await this.prisma.eventTransaction.findMany({
        where: { batchId: id },
      });
  ...
  async getOneChargeSetup(id: string): Promise<any> {
    try {
      const charges = await this.prisma.chargeSetup.findUnique({
        where: { id },
      });
  ...
  async getChargeSetupByEvent(eventId: string): Promise<any> {
    try {
      const charges = await this.prisma.chargeSetup.findFirst({
        where: { eventId },
      });
