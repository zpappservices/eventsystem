const Button = ({
  ...
  return (
    <button
      className={`btn min-h-fit h-fit normal-case relative rounded-[8px] transition-all duration-300 ease-in-out flex items-center justify-center font-medium ${
        isLoading
          ? disabledLoadingStyle
          : "disabled:bg-neutrals200 disabled:text-neutrals500"
      } ${backgroundColor} ${
        sizeClasses[size]
      } ${text} ${outlineClass} ${textOnly} ${style} ${className}`}
      {...props}
    >
      {isLoading ? (
        ...
        <div className="transition-opacity flex items-center gap-2.5 justify-center">
